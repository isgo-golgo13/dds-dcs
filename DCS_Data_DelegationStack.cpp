...
DCS_Data_DelegationStack::DCS_Data_DelegationStack()
: handle_(0)
, application_stack_(0)
{
}
...
bool
DCS_Data_DelegationStack::init_DDS(int& argc, ACE_TCHAR *argv[])
{
  ...
  DCS::DCS_DataDiffTypeSupportImpl* fileinfo_dt =
                new DCS::DCS_DataDiffTypeSupportImpl();
  fileinfo_dt->register_type (dp_.in (),
                              "DCS::DCS_DataDiff");
   topic_ = dp_->create_topic ("dcs_data_topic", // topic name
                               "DCS::DCS_DataDiff", // topic type
                               TOPIC_QOS_DEFAULT,
  ...
  listener_ = new DCS_Data_Listener(this);
  ...
  data_diff_writer_ = DCS::DCS_DataDiffDataWriter::_narrow (dw_.in());
  if (CORBA::is_nil (data_diff_writer_.in ()) ) {
    ACE_ERROR((LM_ERROR,
...
void
DCS_Data_DelegationStack::shutdown_DDS()
{
...
void
DCS_Data_DelegationStack::attach_application(DCS_Data_ApplicationStack* application_stack)
{
  application_stack_ = application_stack;
}


void
DCS_Data_DelegationStack::receive_diff(const DCS::DCS_DataDiff& diff)
{
  ...
  if (0 != application_stack_)
  {
    application_stack_->receive_diff(diff);
  }
  else
  {
    ACE_DEBUG((LM_DEBUG,
      "Warning: application stack received diff while no application defined.\n"));
  }
}


bool
DCS_Data_DelegationStack::send_diff(const DCS::DCS_DataDiff& diff)
{
  // Have the writer publish the diff
  DDS::ReturnCode_t result = data_diff_writer_->write(diff, handle_);
